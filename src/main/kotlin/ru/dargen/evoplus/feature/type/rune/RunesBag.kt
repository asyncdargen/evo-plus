...
object RunesBag {
    ...
    init {
        on<KeyTypeEvent> {
            if (RuneFeature.RunesSetSwitch && CurrentScreen?.title?.string?.contains(MENU_NAME) == true) {
                if (key in InputUtil.GLFW_KEY_1..InputUtil.GLFW_KEY_7) {
                    val index = key - InputUtil.GLFW_KEY_1
                    ...
                    var index = SelectedSet.id + shift
                    when {
    ...
    private fun fetchSelectedRuneSet() =
        RuneSetsSlots.mapNotNull { Player?.currentScreenHandler?.getSlot(it)?.stack?.run { RuneSetsSlots.indexOf(it) to this } }
    ...
    private fun updateRunesProperties() {
        RunesProperties = fetchRawRunesProperties()
            ...
            .toList()
            .toSortedSet(compareByDescending<Pair<*, RuneProperty>> { it.second.value })
            .toMap()

    }

    private fun fetchRawRunesProperties() =
        RunesBagSlots.mapNotNull { Player?.currentScreenHandler?.getSlot(it)?.stack?.takeIf { it.item == Items.PAPER } }
